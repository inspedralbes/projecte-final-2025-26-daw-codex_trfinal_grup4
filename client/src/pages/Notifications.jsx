...
const mapNotificationType = (type) => {
  const typeMap = {
    ...
    solution: "accepted",
    center: "school",
    center_request_approved: "school",
    center_request_rejected: "school",
    center_announcement: "school",
    center_post: "school",
    milestone: "milestone",
...
const formatTimestamp = (dateString) => {
  ...
  if (diffMins < 60) return i18next.t("notifications.time.ago_m", { count: diffMins });
  if (diffHours < 24) return i18next.t("notifications.time.ago_h", { count: diffHours });
  if (diffDays === 1) return i18next.t("notifications.time.yesterday");
  if (diffDays < 7) return i18next.t("notifications.time.ago_d", { count: diffDays });
...
const getIconClass = (type) => {
  ...
  switch (mappedType) {
    case "like":
      return "notif__icon--like";
    case "follow":
      return "notif__icon--follow";
    case "mention":
      return "notif__icon--mention";
    case "school":
      return "notif__icon--school";
    case "accepted":
    case "milestone":
...
export default function Notifications() {
  ...
  return (
    <div className="notifications">
      ...
      <header className="notif__header">
        <div className="notif__header-top">
          <h1 className="notif__title">{t("sidebar.notifications")}</h1>
            ...
            >
              ...
              <div className="notif__content">
                {notif.sender && (
                  <div className="notif__user">
                    <img
                      src={
                        notif.sender.avatar ||
                        `https://api.dicebear.com/7.x/avataaars/svg?seed=${notif.sender.username}`
                      }
                ...
                <div className="notif__body">
                  {mappedType === "school" ? (
                    <>
                      <span className="notif__center">
                        {notif.data?.center_name || t("sidebar.center")}
                      </span>
